...
namespace HyPlayer.Features.Artist
{
    ...
    private readonly ITeachingTipService _teachingTipService;
        ...
        public async Task InitializeArtistInfo(string artistId)
        {
            if (artistId is null)
            {
                _teachingTipService.Enqueue(new("艺人ID为空", "请检查传入的参数是否正确"));
                return;
            }
            var res = await SimpleCacher.GetOrCreateCacheAsync(CacheType.ArtistDetail, artistId, async () =>
            {
                ...
                {
                    _teachingTipService.Enqueue(new("艺人不存在", null));
                    return null;
                }
                if (resp.IsError)
                {
                    _teachingTipService.Enqueue(new("获取艺人信息失败", resp.Error?.Message));
                    return null;
                }
        ...
        private async Task LoadHotSongs()
        {
            ...
            var j1 = await SimpleCacher.GetOrCreateCacheAsync(CacheType.ArtistTopSongsDetail, Artist.Id, async () =>
            {
                ...
                if (j1res.IsError)
                {
                    _teachingTipService.Enqueue(new("获取歌手热门歌曲失败", j1res.Error?.Message));
                    return null;
                }
            ...
            var jv = await SimpleCacher.GetOrCreateCacheAsync(CacheType.SongDetail, Artist.Id, async () =>
            {
                ...
                if (json.IsError)
                {
                    _teachingTipService.Enqueue(new("获取歌手歌曲信息失败", json.Error.Message));
                    return null;
                }
        ...
        private async Task LoadSongs()
        {
            var j1 = await SimpleCacher.GetOrCreateCacheAsync(CacheType.ArtistSongsDetial, Artist.Id + "_" + CurrentPage,
                    async () =>
                    {
                        ...
                        if (resp.IsError)
                        {
                            _teachingTipService.Enqueue(new("获取歌手歌曲失败", resp.Error?.Message));
                            return null;
                        }
        ...
        private async Task LoadAlbum()
        {
            try
            {
                ...
                var jv = await SimpleCacher.GetOrCreateCacheAsync(CacheType.ArtistAlbumsList, Artist.Id + "_" + CurrentPage,
                    async () =>
                    {
                        ...
                        if (resp.IsError)
                        {
                            _teachingTipService.Enqueue(new("获取歌手专辑失败", resp.Error?.Message));
                            return null;
                        }
            ...
            {
                _teachingTipService.Enqueue(new("获取歌手专辑失败", (ex.InnerException ?? new Exception()).Message));
            }
        }
        [RelayCommand]
