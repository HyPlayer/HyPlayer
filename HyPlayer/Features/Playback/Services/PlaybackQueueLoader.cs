...
public sealed class PlaybackQueueLoader : IPlaybackQueueLoader
{
        ...
        INotificationService notification)
    {
        ...
        var byKind = new Dictionary<SongListQueueScopeKind, IQueueSourceProvider>();
        var byPrefix = new Dictionary<string, IQueueSourceProvider>(StringComparer.Ordinal);
        foreach (var provider in queueSourceProviders)
        {
            byKind.TryAdd(provider.Kind, provider);
            byPrefix[provider.Prefix] = provider;
        }
    ...
    {
        cancellationToken.ThrowIfCancellationRequested();
        var songList = songs as List<SingleSongBase> ?? songs.ToList();
        if (songList.Count == 0)
            return false;
