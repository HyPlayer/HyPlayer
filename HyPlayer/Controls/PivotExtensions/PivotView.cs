...
namespace HyPlayer.Controls
{
    ...
    internal class PivotView : Control
    {
        public PivotView()
        {
            this.DefaultStyleKey = typeof(PivotView);
        }
        ...
        {
            get { return (double)GetValue(HeaderScrollProgressProperty); }
        ...
        public static readonly DependencyProperty MaxHeaderScrollOffsetProperty =
            DependencyProperty.Register("MaxHeaderScrollOffset", typeof(double?), typeof(PivotView), new PropertyMetadata(null, (s, a) =>
            {
                if (s is PivotView sender)
                {
                    sender.UpdatePivotMaxHeaderScrollOffset();
        ...
        public static readonly DependencyProperty HeaderHeightProperty =
            DependencyProperty.Register("HeaderHeight", typeof(double), typeof(PivotView), new PropertyMetadata(0d, (s, a) =>
            {
                if (s is PivotView sender)
                {
        ...
        public static readonly DependencyProperty HeaderScrollProgressProperty =
            DependencyProperty.Register("HeaderScrollProgress", typeof(double), typeof(PivotView), new PropertyMetadata(0d, (s, a) =>
            {
                if (s is PivotView sender)
                {
        ...
        private double InternalMaxHeaderScrollOffset => Math.Min(HeaderContainer?.ActualHeight ?? 0, MaxHeaderScrollOffset ?? double.MaxValue);
        ...
        protected override void OnApplyTemplate()
        {
            ...
            {
                PivotExHeaderView.SizeChanged -= PivotExHeaderView_SizeChanged;
            }
            if (PivotContainer != null)
            {
                PivotContainer.SizeChanged += PivotContainer_SizeChanged;
            }
        ...
        private void UpdatePivotClip()
        {
            if (PivotContainer != null)
            {
                ...
                {
                    var y = (PivotExHeaderView?.ActualHeight ?? 0) + (HeaderContainer?.ActualHeight ?? 0) - InternalMaxHeaderScrollOffset;
                    clip.Rect = new Windows.Foundation.Rect(0, y, PivotContainer.ActualWidth, Math.Max(0, PivotContainer.ActualHeight - y));
                }
                else
                {
                    clip.Rect = new Windows.Foundation.Rect(0, 0, PivotContainer.ActualWidth, PivotContainer.ActualHeight);
                }
            }
        }
