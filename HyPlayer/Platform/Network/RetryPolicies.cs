...
public static class RetryPolicies
{
    ...
    public static AsyncRetryPolicy MediaSourceLoadPolicy { get; } = Policy
        ...
        .WaitAndRetryAsync(
            5,
            attempt => TimeSpan.FromMilliseconds(600 * attempt),
            (exception, timespan, retryCount, context) =>
            {
                Debug.WriteLine(
    ...
    public static AsyncRetryPolicy FileAccessPolicy { get; } = Policy
        ...
        .WaitAndRetryAsync(
            2,
            attempt => TimeSpan.FromMilliseconds(150 * attempt),
            (exception, timespan, retryCount, context) =>
            {
                Debug.WriteLine(
    ...
    public static AsyncRetryPolicy ApiCallPolicy { get; } = Policy
        ...
        .WaitAndRetryAsync(
            2,
            attempt => TimeSpan.FromSeconds(Math.Pow(1.5, attempt)), // 较温和的指数退避
            (exception, timespan, retryCount, context) =>
            {
                Debug.WriteLine(
        ...
        T fallbackValue = default!)
    {
        ...
        {
            Debug.WriteLine($"操作失败，使用回退值: {ex.Message}");
