...
namespace HyPlayer.Classes
{
    ...
    class CloudUpload
    {
        public static async Task<Dictionary<string, JObject>> UploadMusic(StorageFile file)
        {
            ...
            var (isOk, res) = await Common.ncapi.RequestAsync(CloudMusicApiProviders.CloudUploadCheck,
            ...
            var (ok1, tokenres) = await Common.ncapi.RequestAsync(CloudMusicApiProviders.CloudUploadToken,
                new Dictionary<string, object>()
                {
                    {"ext", file.FileType},
                    {"filename", file.Name},
            ...
            if (isOk && res["needUpload"].ToObject<bool>())
            {
                // 文件需要上传
                var (ok0, tokenRes) = await Common.ncapi.RequestAsync(CloudMusicApiProviders.CloudUploadToken,
                    new Dictionary<string, object>()
                    {
                        {"ext", file.FileType},
                        {"filename", file.Name},
            ...
            string title = string.IsNullOrEmpty(musicprop.Title)
                ...
                : musicprop.Title;
            var (ok2, res2) = await Common.ncapi.RequestAsync(CloudMusicApiProviders.UploadCloudInfo,
                new Dictionary<string, object>()
                {
                    {"md5", md5},
            ...
            var (ok3, res3) = await Common.ncapi.RequestAsync(CloudMusicApiProviders.CloudPub,
                new Dictionary<string, object>()
                {
                    {"songid", res2["songId"]}
                });
