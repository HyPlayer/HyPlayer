...
public sealed class NeteaseQueueSourceLoadResult
{
    ...
    public static NeteaseQueueSourceLoadResult FromSongs(IList<SingleSongBase> songs) => new()
    {
        Success = true,
        Batches = songs is { Count: > 0 } ? [songs] : []
    };

    public static NeteaseQueueSourceLoadResult FromBatches(IList<IList<SingleSongBase>> batches) => new()
    {
        Success = true,
