                ...
                using (var clds = cl.CreateDrawingSession())
                {
                    ...
                    if (_isFocusing)
                    {
                        if (IsSyllable || context.Effects.SimpleLineScanning)
                        {
                            ...
                            // before
                            var beforeCommandList = new CanvasCommandList(clds);
                            using var beforeDrawingSession = beforeCommandList.CreateDrawingSession();
                            using (beforeDrawingSession.CreateLayer(1, beforeCurrentSyllableGeometry, matrix))
                            {
                                beforeDrawingSession.DrawTextLayout(textLayout, 0, textTop - lift,
                                    TypographySelector(t => t?.FocusingColor, context)!.Value);
                                // Colors.Red);
                            }

                            // after
                            var afterCommandList = new CanvasCommandList(clds);
                            using var afterDrawingSession = afterCommandList.CreateDrawingSession();
                            using (afterDrawingSession.CreateLayer(1, afterCurrentSyllableGeometry, matrix))
                            {
                                afterDrawingSession.DrawTextLayout(textLayout, 0, textTop,
                                    TypographySelector(t => t?.IdleColor, context)!.Value);
                                // Colors.Green);
                            }


                            clds.DrawImage(beforeCommandList, 0, 0);
                            clds.DrawImage(afterCommandList, 0, 0);
