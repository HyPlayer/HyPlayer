...
public class SinRollingCalculator : LineRollingCalculator
{
        ...
        RenderContext context)
    {
        ...
        var progress = Math.Clamp((context.CurrentLyricTime - context.CurrentKeyframe) * 1.0f / AnimationDuration, 0,
            1);

        if (!(fromY < targetY) && gap >= 0)
            // 往下走, 用正弦函数
            progress = Math.Clamp(
                (context.CurrentLyricTime - context.CurrentKeyframe) * 1.0f /
                (AnimationDuration * ((float)Math.Log10(Math.Max(gap, 0.9)) + 1)), 0, 1);
