...
public sealed class LyricRenderView
{
    ...
    private readonly CanvasTransition _scrollTranslation = new()
    {
    ...
    public bool HasJumpedLyrics { get; private set; }
    ...
    public void SetLyricLines(List<RenderingLyricLine> lines)
    {
        ...
        for (var i = 0; i < Context.LyricLines.Count; i++) Context.LyricLines[i].Id = i;
        ...
        // 初始化位置
        var topleftPosition = Context.ViewHeight * Context.LyricPaddingTopRatio;
    ...
    public void Clear()
    {
        foreach (var line in Context.LyricLines) line.Dispose();
    ...
    private void RecalculateRenderOffset(CanvasDrawingSession session)
    {
        ...
        Context.RenderingLyricLines.Clear();
        var theoryRenderAfterPosition = Context.LyricPaddingTopRatio * Context.ViewHeight +
                                        _scrollTranslation.Animate(Context.CurrentLyricTime, Context.ScrollingDelta);
    ...
    public void Draw(CanvasDrawingSession session, CanvasTimingInformation timing)
    {
        try
        {
            ...
            foreach (var key in _keyFrameRendered.Keys.ToArray())
            {
                ...
                // 视图快照
                //if (!_needRecalculate)
                foreach (var (i, value) in Context.RenderOffsets) Context.SnapshotRenderOffsets[i].Y = value.Y;


                // 0 时刻渲染所有, 也就是初始化
                var targets = key == 0 ? Context.LyricLines : _targetingKeyFrames[key];
                foreach (var renderingLyricLine in targets) renderingLyricLine.OnKeyFrame(session, Context);
    ...
    public void LyricView_OnPointerMoved(object sender, PointerRoutedEventArgs e)
    {
        ...
        if (e.Pointer.PointerDeviceType == PointerDeviceType.Mouse)
        {
            var focusingLine = -1;
            var firstPosition = 0;
            var lastPosition = Context.RenderOffsets.Count - 1;
            var maximumAttempts = (int)Math.Ceiling(Math.Log(lastPosition + 1)) + 3;
            var attemptCount = 1;
            while (attemptCount <= maximumAttempts)
            {
                attemptCount += 1;
                var renderOffsetsKey = (firstPosition + lastPosition) / 2;
                ...
                if (lastPosition - firstPosition == 1) //结束时刻前面是下取整，两个都不是那就是没移到上面
                {
                    if (Context.RenderOffsets[renderOffsetsKey + 1].Y <=
                        e.GetCurrentPoint((UIElement)sender).Position.Y &&
        ...
        else if (_pointerPressed && _lastPointerPressedYValue != null)
        {
