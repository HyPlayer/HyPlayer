...
public sealed class RectRevealCalculator
{
    public float GetRevealProgress(LyricGlyphCluster cluster, TextRenderFrame frame)
    {
        if (cluster.Layer == LyricTextLayer.Transliteration)
        {
            ...
            {
                return GetLineProgress(cluster, frame);
            }
    ...
    {
        if (cluster.LayerClusterCount <= 0)
        {
            return Math.Clamp(frame.LineProgress, 0, 1);
        }

        if (cluster.VisualLineClusterCount <= 0)
        {
            return Math.Clamp(frame.LineProgress, 0, 1);
        }
        ...
        if (cluster.VisualLineIndex < currentLine)
        {
            return 1;
        }
