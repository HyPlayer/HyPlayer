...
public static class KaraokeParser
{
    public static ILyricCollection ParseKaraoke(ReadOnlySpan<char> input)
    {
        ...
        var karaokeWordInfos = new List<KaraokeWordInfo>();
        ...
        for (var i = 0; i < input.Length; i++)
            if (i != input.Length)
            {
                ...
                if (curChar == '\n' || curChar == '\r' || i + 1 == input.Length)
                {
                    ...
                    {
                        if (input[i + 1] == '\n' || input[i + 1] == '\r') i++;
                        karaokeWordInfos.Add(new KaraokeWordInfo(lyricStringBuilder.ToString(),
                            TimeSpan.FromMilliseconds(wordTimespan), TimeSpan.FromMilliseconds(wordDuration)));
                        lines.Add(new KaraokeLyricsLine(karaokeWordInfos, lyricWithoutPuncBuilder.ToString(),
                            TimeSpan.FromMilliseconds(lyricTimespan), TimeSpan.FromMilliseconds(lyricDuration)));
                    ...
                    if (i + 1 == input.Length) reachesEnd = true;
                }
                ...
                switch (state)
                {
                    ...
                    case CurrentState.Lyric:
                        ...
                        if (!char.IsPunctuation(curChar) && !char.IsWhiteSpace(curChar))
                            lyricWithoutPuncBuilder.Append(curChar);
                        break;
                }
