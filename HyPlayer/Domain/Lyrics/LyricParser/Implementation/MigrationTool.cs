...
{
    private static readonly Cosine _cosine = new();
#nullable enable
    private static LyricLine? FindLyric(LyricLine lyricLine, ILyricCollection collection, double similarity, int range)
    {
        ...
        if (sameTime.Any()) return sameTime.First();

        var lyrics = collection.Lines
            .Where(t => Math.Abs((t.StartTime - lyricLine.StartTime).TotalMilliseconds) <= range).ToList();
        if (!lyrics.Any()) return default;
...
#nullable restore
    public static MigrateCollection Migrate(ILyricCollection target, ILyricCollection source, double similarity = 0.80,
        int range = 750)
    {
