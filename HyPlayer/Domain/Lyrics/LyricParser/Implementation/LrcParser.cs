...
public static class LrcParser
{
    public static LrcLyricCollection ParseLrc(ReadOnlySpan<char> input)
    {
        ...
        for (var i = 0; i < input.Length; i++)
        {
            ...
            {
                if (curChar != '\n' && curChar != '\r' && i + 1 < input.Length) continue;
            ...
            if (reachesEnd && state == CurrentState.Lyric)
            {
                for (var j = 0; j < curTimestamps.Length; j++)
                {
                    if (curTimestamps[j] == -1) break;
                    lines.Add(new LrcLyricsLine(
                        input.Slice(curStateStartPosition + 1,
                            i - curStateStartPosition - (lastCharacterIsLineBreak ? 1 : 0)).ToString(),
                        TimeSpan.FromMilliseconds(curTimestamps[j] - offset)));
                }
            ...
            switch (state)
            {
                ...
                case CurrentState.None:
                    ...
                    if (curChar == '[') state = CurrentState.AwaitingState;
                ...
                case CurrentState.AttributeContent:
                    ...
                    ;
                    if (attributeName == "offset") timeCalculationCache = timeCalculationCache * 10 + curChar - '0';
                    break;
