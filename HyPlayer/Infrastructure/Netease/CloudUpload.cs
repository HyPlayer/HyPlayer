...
#nullable enable
    public static async Task UploadMusic(StorageFile file)
    {
        ...
        teachingTipService.Enqueue(new("上传本地音乐至音乐云盘中", "正在上传: " + file.DisplayName));
        ...
        {
            teachingTipService.Enqueue(new($"上传失败: {file.DisplayName}", checkResult.Error!.Message));
            return;
        }
        ...
        if (checkResult.Value?.NeedUpload is not false)
        {
            ...
            if (tokenRes.IsError)
            {
                teachingTipService.Enqueue(new($"上传失败: {file.DisplayName}", tokenRes.Error!.Message));
                return;
            }
        ...
        if (coverBytes != null)
        {
            ...
            {
                teachingTipService.Enqueue(new($"上传失败(封面): {file.DisplayName}", coverAllocRes.Error!.Message));
            }
        ...
        if (infoRes.IsError)
        {
            teachingTipService.Enqueue(new($"上传失败: {file.DisplayName}", infoRes.Error!.Message));
            return;
        }
        ...
        {
            teachingTipService.Enqueue(new($"上传失败: {file.DisplayName}", cloudPubRes.Error!.Message));
        }
        else
        {
            teachingTipService.Enqueue(new("上传本地音乐至音乐云盘成功", "成功上传: " + file.DisplayName));
        }
